...
class AddChildren extends Component {
  constructor(props) {
    super(props);
    this.state = {
      account: { name: '', username: '', password: '', DOB: '', parentUsername: '' }
    }
  }
  componentWillMount() {
  ...
  updateParentUsername = (event) => {
    let account = this.state.account;
    account.parentUsername = event.target.value;
    this.setState({ account: account });
  }
  updateDOB = (event) => {
    let account = this.state.account;
  ...
  saveAccount = (event) => {
    ...
    if (!account.username.trim() || !account.password.trim() || !account.DOB.trim()
          || !account.name.trim() || !account.parentUsername.trim()) {
      ...
      <div>
        <div className="add-children-container">
          ...
          <div className="add-children-information">
            ...
            <div className="form-group form-inline">
              <label htmlFor="cartID">Parent Username
            </label>
              <input disabled={this.props.loading} type="text" className="form-control" name="cartID"
                onChange={(event) => { this.updateParentUsername(event) }} value={this.state.account.parentUsername}
                style={{ width: '80%' }} />
            </div>
            <div className="form-group form-inline">
